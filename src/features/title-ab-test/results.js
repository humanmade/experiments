...
export const ResultWithData = compose(
	withSelect(select => {
		return {
			...
			results: select('core/editor').getCurrentPostAttribute('meta').ab_tests.titles.results || {},
			endTime: select('core/editor').getCurrentPostAttribute('meta').ab_tests.titles.end_time,
		};
	})
)(Results);
